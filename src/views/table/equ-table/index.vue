    ...
    <section class="home">
      ...
      <el-col :span="24" class="toolbar" style="padding: 10px 20px 0px;">
        <el-form :inline="true" :model="filters">
          ...
          <el-form-item>
            <el-select v-model="filters.status" filterable placeholder="请选择设备状态" clearable>
      ...
      <!--列表-->
      <el-table v-loading="listLoading" highlight-current-row :data="search(keywords)" style="width: 100%" @selection-change="selsChange">
      ...
      <el-col :span="24" class="toolbar">
        <el-button type="danger" @click="batchRemove">批量处理</el-button>
      ...
      <el-dialog :title="addFlag? '新增':'修改'" :visible.sync="modalShow" :before-close="modalClose">
        <el-form ref="form" :model="form" label-width="80px">
          ...
          <el-form-item label="类别">
            <el-select v-model="form.category" placeholder="选择设备类别">
              <el-option label="传感器类" value="1" />
              <el-option label="电气试验类" value="2" />
              <el-option label="消防试验类" value="3" />
              <el-option label="液压试验类" value="4" />
              <el-option label="其他类" value="5" />
            </el-select>
          </el-form-item>
...
import equEchart from './Echart/equEchart'
...
export default {
  components: { loanRegister, equEchart },
  data() {
    return {
      filters: {
        name: '',
  ...
  methods: {
    detail(row) {
      if (row.equ_status === '借出') {
        this.$axios.get('api/equ/loan_msg/', { params: { equ_num: row.serialNum }}).then((res) => {
            ...
            for (var key in ValueMap) {
              if (value === key) {
                ...
                if (valueList.indexOf(value) === -1) {
                  ...
                  dict.key = value
                  dict.display_name = ValueMap[key]
                  EquStatusOption.push(dict)
                }
              }
            }
          }
    ...
    searchHandle: function(params) {
      getEqu(params).then((res) => {
        ...
        if (response.error_num === 0) {
          var equList0 = response['list']
          console.log(equList0)
          if (equList0 !== undefined) {
            for (var i = 0; i < equList0.length; i++) {
              var value = equList0[i].equ_status
              for (var key in ValueMap) {
                if (value === key) {
